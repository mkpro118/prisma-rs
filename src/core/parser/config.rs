...
///
/// - `max_errors`: stop adding errors after this many have been recorded.
/// - `strict_mode`: tighten grammar checks; implementations may fail on
///   ambiguities instead of recovering.
/// - `allow_experimental`: enable gated language features.
/// - `error_recovery`: enable recovery strategy to continue after errors.
/// - `enable_multithreading`: enable multi-threaded parsing for top-level declarations.
/// - `max_threads`: maximum number of threads to use for parsing.
/// - `concurrency_threshold`: minimum number of top-level items to trigger concurrent parsing.
